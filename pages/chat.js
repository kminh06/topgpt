  ...
  const [session, setSession] = useState([
    {
      role: 'system',
      content:
        'You are TopGPT, a helpful assistant powered by OpenAI and created by Khac Minh Dau. You will only give answers in 500 words or less.',
    },
  ])
      ...
      onSnapshot(docRef, (doc) => {
        if (doc.data() === undefined) {
          setDoc(docRef, {
            ...
            date_joined: new Intl.DateTimeFormat('en-US', {
              month: 'short',
              day: 'numeric',
              year: 'numeric',
            }).format(new Date()),
  ...
  function handleSubmit(text) {
    ...
    openai
      ...
      .catch((error) => {
        ...
        setSession([
          ...session,
          { prompt: text, content: 'Could not answer, please try again' },
        ])
      })
  }
    ...
    if (message.role === 'user') {
      return (
        <div className='message'>
          <span>
            <img alt='user' id='user' className='pic' src={user.photoURL} />
            {message.content}
          </span>
        </div>
      )
          ...
          {user.has_paid ? (
            ...
            <>
              <input
                ...
                disabled='disabled'
                autoComplete='off'
                placeholder='Please make payment to chat'
